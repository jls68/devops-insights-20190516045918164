...
(function () {
    ...
    describe('Get Weather', function() {
        ...
        request({
              ...
              assert((pbody.city === 'Auckland') || (pbody.city === 'Auckland,nz'), "City name does not match");
              done();
            }
        });
    	});
      ...
      it('with another valid city name', function(done) {
        ...
        request({
      		method: 'GET',
              url: appUrl + '/api/v1/getWeather?q=Taupo'
          }, function(err, resp, body) {
          	if(err) {
          		assert.fail('Failed to get the response');
          	} else {
              assert.equal(resp.statusCode, 200);
              var pbody = JSON.parse(body);
              assert(pbody.city === 'Taupo', "City name does not match");
              done();
            }
        });
    	});
